...
(function () {
  ...
  if (!site || site.indexOf('http') !== 0)
    site =
      (window.PORTFOLIO_META && window.PORTFOLIO_META.siteUrl) || 'https://brokeCode05.github.io/portfolio/';
  var qr = document.getElementById('hero-badge-qr-real');
  if (qr) {
    qr.src =
      'https://api.qrserver.com/v1/create-qr-code/?size=160x160&margin=8&data=' + encodeURIComponent(site);
  }
  ...
  var TILT_X = 4,
    TILT_Y = -3; // resting tilt (deg), mirrors the old static pose
  var curX = TILT_X,
    curY = TILT_Y;
  var targetX = TILT_X,
    targetY = TILT_Y;
