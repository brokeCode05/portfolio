...
function renderTechStack(data) {
  ...
  rowOrder.forEach(function(cat, ri) {
    ...
    html += '<div class="tech-marquee-row ' + dir + '" data-cat="' + escapeHtml(cat) + '"' + extraAttr + '>' +
...
function renderProjects(data) {
  ...
  return data.projects.map(function(proj, idx) {
    ...
    return '<article class="project-card" ' + tagsAttr + ' data-reveal data-reveal-delay="' + (100 + idx * 100) + '">' +
  ...
  (function(){
    ...
    (function loadAndRender(){
      ...
      function renderPortfolio(data) {
        ...
        if (badgeImg) {
          badgeImg.src = (data.hero.photo || (window.PORTFOLIO_META && window.PORTFOLIO_META.photo) || 'img/johnbryan.jpg');
          badgeImg.style.display = 'block';
        }
        var badgeName = document.getElementById('hero-badge-name');
        if (badgeName) badgeName.textContent = data.hero.name || (window.PORTFOLIO_META && window.PORTFOLIO_META.name) || 'John Bryan Capellan';
        var badgeRole = document.getElementById('hero-badge-role');
        if (badgeRole) badgeRole.textContent = data.hero.badge || (window.PORTFOLIO_META && window.PORTFOLIO_META.role) || 'Information Technology Student';
        ...
        if (badgeIdno) badgeIdno.textContent = data.hero.idNumber || (window.PORTFOLIO_META && window.PORTFOLIO_META.idNumber) || 'IT-2024-0842';
        var personName = data.hero.name || (window.PORTFOLIO_META && window.PORTFOLIO_META.name) || 'John Bryan Capellan';
        ...
        var schoolTitle = data.hero.schoolTitle || (window.PORTFOLIO_META && window.PORTFOLIO_META.schoolTitle) || 'Student ID';
        var schoolSub = data.hero.schoolSub || (window.PORTFOLIO_META && window.PORTFOLIO_META.schoolSub) || 'brokeCode05.dev';
        ...
        if (badgeBackId) badgeBackId.textContent = data.hero.idNumber || (window.PORTFOLIO_META && window.PORTFOLIO_META.idNumber) || 'IT-2024-0842';
      ...
      if (typeof fetchFromSupabase === 'function') {
        ...
        fetchFromSupabase()
          .then(function(result) {
            if (resolved) { dbg('[cloud-sync] Already resolved, skipping'); return; }
            resolved = true;
            ...
            dbg('[cloud-sync] Cloud data received:', cloudData ? 'keys=' + Object.keys(cloudData).join(',') : 'null');
            ...
            if (cloudData && Object.keys(cloudData).length > 3) {
              ...
              dbg('[cloud-sync] Cloud data rejected: keys=' + (cloudData ? Object.keys(cloudData).length : 0) + ' (need > 3)');
            }
